...
void count_sort(char * _dat, int * _in_idx, int * _out_idx, int _in_out_sz, int * _cnt_b, int * _cnt_e) {
    std::memset(_cnt_b, 0, sizeof(int) * (_cnt_e - _cnt_b)); // обнуление счетчика
    ...
    if (*_cnt_b == _cnt_e - _cnt_b) {
        std::memcpy(_out_idx, _in_idx, _in_out_sz * sizeof(int));
        return;
    }
...
int main() {
    ...
    auto buff = new char[m + 1];
    char * dat_pt = dat;
    for (auto row = 0; row < m; ++row) {
        edx_next_bounded(buff, m + 1);

        for (auto pt = buff; pt != buff + m; ++pt) {
            *(dat_pt++) = *pt;
        }
    }

    auto r = radix_sort(dat, dat + dat_len, n, k);

    for (auto pt = r; pt != r + m; ++pt) {
        edx_print_i32(*pt + 1);
        PRINT_SPACE;
    }
